...
class Database {
   ...
   * @constructor
   * @param {string} directory - Путь к директории, где хранятся JSON-файлы пользователей.
   */
  ...
  /**
   * Записывает данные пользователя атомарно.
   * Ошибка записи не подавляется, чтобы вызывающий код не продолжал транзакцию
  ...
  async #writeUser(id, data) {
    if (!data) {
      throw new Error(`Попытка записи пустых данных для пользователя ${id}`);
    }
