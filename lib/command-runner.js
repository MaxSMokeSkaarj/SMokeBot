...
/**
 * Executes a command using the common BotContext contract and persists account
 * changes while holding locks for all accounts involved in the command.
 *
...
const executeCommand = async (context) => {
  ...
  try {
    return await users.withLocks(userIds, async ({ read, write }) => {
