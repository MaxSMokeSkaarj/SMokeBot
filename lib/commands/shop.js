...
const buy = async (itemType, choiceIndex) => {
  ...
  if (!item || !item.name || !item.tier[0].price) {
  ...
  if (isArrayOfIndexes) {
    if (itemType === 'бизнес') {
      ...
      ctx.account[field].push({ id: choiceIndex - 1, timeout, tier: 1, income: items[choiceIndex - 1].tier[0].income });
    } else {
...
const sell = async (itemType, choiceIndex) => {
  const { file, field, isArrayOfIndexes } = itemTypes[itemType];
  const items = await loadItems(file);
  ...
  const sellPrice = Number((item.price * 0.6).toFixed(2)); // 60% от цены покупки
...
export const command = async (ctx) => {
  ...
  try {
    if (command === 'купить') {
      return await buy(itemType, choiceIndex);
    } else if (command === 'продать') {
      return await sell(itemType, choiceIndex);
