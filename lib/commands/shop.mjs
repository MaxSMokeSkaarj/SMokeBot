...
const itemTypes = {
  дом: { file: 'json/houses.json', field: 'houses', isArrayOfIndexes: true },
  транспорт: { file: 'json/cars.json', field: 'cars', isArrayOfIndexes: true },
  телефон: { file: 'json/phones.json', field: 'phones', isArrayOfIndexes: true },
  бизнес: { file: 'json/buisnesses.json', field: 'buisnesses', isArrayOfIndexes: false },
  питомец: { file: 'json/pets.json', field: 'pets', isArrayOfIndexes: true },
};
...
async function sellItem(user, itemType, choiceIndex, ctx) {
  ...
  user.money += Math.floor(item.price * 0.8); // FIXME
  users.update(user.id, { ...user });
