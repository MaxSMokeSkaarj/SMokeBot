...
import { getActiveEconomyTransaction } from './economy-transaction.js';
...
class Bank {
  ...
  async addMoney(amount) {
    const transaction = getActiveEconomyTransaction();
    if (transaction) {
      if (typeof amount !== 'number' || amount < 0) throw new Error('Некорректная сумма для добавления');
  ...
  async removeMoney(amount) {
    const transaction = getActiveEconomyTransaction();
    if (transaction) {
      if (typeof amount !== 'number' || amount < 0) throw new Error('Некорректная сумма для списания');
      transaction.bank.balance -= amount;
      return transaction.bank.balance;
    }
  ...
  async tryTransferMoney(inAmount, outAmount) {
    const transaction = getActiveEconomyTransaction();
    if (transaction) {
      if (typeof inAmount !== 'number' || inAmount < 0 ||
          typeof outAmount !== 'number' || outAmount < 0) {
        throw new Error('Некорректная сумма для перевода');
      }
